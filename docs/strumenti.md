...
| `cutscene-sheet.scene` | **il provino dei pannelli** (§5.27): ferma il loop e ridisegna tutta la cutscene in una griglia sola. Serve a **trovare** i pannelli illeggibili, non a giudicarne il dettaglio — e li trova, perché nel sorgente un nero su nero si legge come un contrasto. Va lanciata larga (`--size 1800x1500`) |
| `cutscene-panel.scene` | **un pannello solo**, fermo, a tutto schermo, per guardarlo davvero. Il numero è quello del copione (1-28) e si passa con uno script di una riga davanti: `echo 'window.__PANEL = 11;' > /tmp/p.js`, poi `--script /tmp/p.js --script …/cutscene-panel.scene` |
