...
| `cutscene-sheet.scene` | **i pannelli di una scena in una griglia sola.** Uno scatto, e si vede quale tavola è una macchia e quale ha il testo sopra la faccia. Di suo mostra l'apertura (si lancia con `--menu`); con `echo 'window.__SCENA="m2";'` davanti mostra le tavole di una missione o del 병원 — `intro` · `m1` · `m2` · `ospedale` |
