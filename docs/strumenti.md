...
| `cutscene-sheet.scene` | **i pannelli di una scena in una griglia sola.** Uno scatto, e si vede quale tavola è una macchia e quale ha il testo sopra la faccia. Di suo mostra l'apertura (si lancia con `--menu`); con `echo 'window.__SCENA="m2";'` davanti mostra le tavole di una missione, di un raccordo o del 병원 — `intro` · `m1` · `m2` · `m3` · `m4` · `r1` · `ospedale` |
| `missioni-run.scene` | **M1 e M2 giocate davvero**, dall'auto ai pannelli di chiusura (§5.29), e *asserisce*: fasi che si chiudono, blip che si sposta, porta tenuta aperta fuori orario, serranda sigillata, ripresa dall'ultima fase dopo una morte, salvataggio che si ricorda a che punto è la storia, i tre pegni, la rapina, e le battute del 병원 che non si ripetono. Si lancia filtrando i `false`, e `[]` vuol dire tutto a posto; `window.__LOG` è la fila delle fasi attraversate |
| `atto-1.scene` | **M3, M4 e R1 giocati davvero** (§5.33), e *asserisce*: l'appuntamento delle quattro aspettato invece che guardato, le due auto che arrivano davvero dove sono mandate, il pedinamento che si perde restando indietro e salta stando addosso, il volume che scende quando Kkachi lo chiede, i fogli che restano per terra dove sei caduto e tornano dal salvataggio, Oh Se-jung che muore senza un pannello, la camera della gru, le tre stelle della fuga in barca e uno slot v5 che si migra. Si lancia filtrando i `false`, e `[]` vuol dire tutto a posto; cinque voci sono invarianti — `unBlipSolo`, `corriereArriva`, `pedinamentoSiPerde`, `fogliRestanoDoveSeiCaduto`, `fogliTornanoDalSalvataggio` |
| `itinerario.scene` | **la strada verso il blip** (§5.30), e *asserisce*: il percorso parte dal giocatore e finisce *sulla* meta, non è più corto della linea d'aria né più lungo del doppio e mezzo, non fa salti fra due punti consecutivi, Jeju resta in linea d'aria perché su gomma non ci si arriva, dentro un negozio non si ricalcola, e senza blip non resta una linea appesa. Misura anche il caso peggiore (Seoul → Busan): sopra il mezzo millisecondo, l'itinerario ha smesso di essere gratis. Si lancia filtrando i `false`, e `[]` vuol dire tutto a posto |
| `kkachi.scene` | **까치 sulla `91.45`** (§5.32), e *asserisce*: la stazione che c'è prima della rete e non ne ha bisogno, il silenzio a piedi, la radio vera che vince, una riga di servizio salendo in macchina con l'ora **vera** dentro, una chiamata innescata guidandoci sopra davvero, la chiamata persa scendendo che non torna mai più, il silenzio sotto le stelle, la battuta di trama che a radio spenta non la dice nessuno, il round-trip del salvataggio e uno slot v4 che si migra. Si lancia filtrando i `false`, e `[]` vuol dire tutto a posto; quattro voci sono invarianti — `nienteReteSuNoveUnoQuattroCinque`, `zittoAPiedi`, `persaNonTornaMai`, `zittoSottoLeStelle` |
| `cortili.scene` | **i cortili che cambiano padrone** (§5.31), e *asserisce*: la barra della presa che sale, il tag che cambia colore, il banco che passa al listino del padrone nuovo, la busta che cresce sull'orologio e si ferma al tetto, il round-trip del salvataggio, uno slot v3 che si migra invece di essere buttato, l'acquisto che paga davvero, la guerra persa che riporta indietro il cortile. Si lancia filtrando i `false`, e `[]` vuol dire tutto a posto; tre voci sono invarianti — `restaTuoDopoRicarica` (è **la** prova della tappa C), `nuovaPartitaRimetteLaSeed`, `bustaHaUnTetto` |
| `cutscene-panel.scene` | **un pannello solo, fermo, a grandezza vera** — è qui che si capisce *perché* una tavola non si legge, e l'unico modo di vedere che il riquadro delle battute copre una faccia o esce dal pannello (§5.33). Si sceglie con uno script di una riga davanti: `echo 'window.__PANEL = 11;' > /tmp/p.js`, e con `window.__SCENA` (`m1` … `r1`, `ospedale`) `__PANEL` diventa l'id del pannello (`'m4-7'`) o la sua posizione |
