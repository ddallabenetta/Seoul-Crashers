...
/**
 * Una sagoma umana. `s` è l'altezza in pixel; le pose sono quelle che il copione
 * chiede davvero, e si aggiungono qui quando un pannello ne vuole una nuova.
 *
 * Nessun tratto sul viso: dove servirebbe un'espressione, la scena è costruita
 * perché non serva (regola della cutscene).
 */
export function figure(P, x, y, s, pose = 'stand', color = INK, opts = {}) {
  const ctx = P.ctx;
  const { flip = false, coat = null } = opts;
  ctx.save();
  ctx.translate(x, y);
  ...
  const torso = (topW, botW, top, bot) => {
    ctx.beginPath();
    ctx.moveTo(-topW, -s * top);
    ctx.lineTo(topW, -s * top);
    ctx.lineTo(botW, -s * bot);
    ctx.lineTo(-botW, -s * bot);
    ctx.closePath();
    ctx.fill();
  };
  const dot = (cy, r = head) => { ctx.beginPath(); ctx.arc(0, -s * cy, r, 0, Math.PI * 2); ctx.fill(); };
  const leg = (dx, dy = 0) => ctx.fillRect(dx - s * 0.045, -s * 0.46, s * 0.09, s * 0.46 + dy);

  switch (pose) {
    case 'kneel':
      // Il capo chino in avanti è quello che la fa leggere come una persona
      // inginocchiata invece che come una pedina degli scacchi.
      ctx.save();
      ctx.rotate(0.14);
      dot(0.56, head * 0.9);
      torso(s * 0.12, s * 0.17, 0.47, 0.16);
      ctx.restore();
      ctx.fillRect(-s * 0.2, -s * 0.16, s * 0.4, s * 0.11);    // le gambe piegate
      ctx.fillRect(-s * 0.27, -s * 0.05, s * 0.13, s * 0.05);  // il piede che sporge
      break;
    case 'seat':
      dot(0.72, head * 0.92);
      torso(s * 0.12, s * 0.17, 0.63, 0.3);
      ctx.fillRect(-s * 0.06, -s * 0.32, s * 0.3, s * 0.09);   // coscia
      ctx.fillRect(s * 0.15, -s * 0.32, s * 0.09, s * 0.32);   // stinco
      break;
    case 'curb':
      dot(0.62, head * 0.92);
      torso(s * 0.12, s * 0.16, 0.53, 0.22);
      ctx.fillRect(-s * 0.02, -s * 0.24, s * 0.26, s * 0.08);
      ctx.fillRect(s * 0.17, -s * 0.24, s * 0.08, s * 0.24);
      break;
    case 'hug':   // Chun-sik che apre le braccia
      dot(0.86, head * 1.05);
      torso(s * 0.2, s * 0.26, 0.74, 0);
      ctx.save();
      ctx.rotate(-0.5); ctx.fillRect(-s * 0.06, -s * 0.72, s * 0.12, s * 0.4); ctx.restore();
      ctx.save();
      ctx.rotate(0.5); ctx.fillRect(-s * 0.06, -s * 0.72, s * 0.12, s * 0.4); ctx.restore();
      break;
    case 'back':  // di spalle, il bomber addosso
      dot(0.88, head);
      torso(s * 0.15, s * 0.17, 0.76, 0.44);
      leg(-s * 0.07); leg(s * 0.07);
      if (coat) {
        ctx.fillStyle = coat;
        ctx.fillRect(-s * 0.155, -s * 0.6, s * 0.31, s * 0.05);
      }
      break;
    case 'walk':
      dot(0.88, head);
      torso(s * 0.12, s * 0.15, 0.76, 0.44);
      ctx.save(); ctx.rotate(0.22); leg(0); ctx.restore();
      ctx.save(); ctx.rotate(-0.18); leg(0); ctx.restore();
      break;
    case 'lean':
      dot(0.86, head);
      ctx.save(); ctx.rotate(0.13);
      torso(s * 0.12, s * 0.15, 0.74, 0.44);
      ctx.restore();
      leg(-s * 0.06); leg(s * 0.08);
      break;
    default:      // in piedi, fermo
      dot(0.88, head);
      torso(s * 0.13, s * 0.16, 0.76, 0.44);
      leg(-s * 0.07); leg(s * 0.07);
      if (coat) {
        ctx.fillStyle = coat;
        ctx.fillRect(-s * 0.14, -s * 0.62, s * 0.28, s * 0.045);
      }
  }
  ctx.restore();
}

/** Una fila di sagome, per una folla che non conta come personaggi. */
export function crowd(P, x, y, s, n, opts = {}) {
  const { spread = s * 0.9, color = 'rgba(9,11,15,0.9)', pose = 'stand', seed = 9 } = opts;
  for (let i = 0; i < n; i++) {
    const k = hash(seed * 13 + i * 29);
    figure(P, x + (i - (n - 1) / 2) * spread + (k - 0.5) * s * 0.2,
      y + (k - 0.5) * s * 0.08, s * (0.88 + k * 0.24), pose, color, { flip: k > 0.5 });
  }
}

/**
 * Il cane. Non esiste nel gioco e non esisterà: serve in due pannelli fermi
 * (l'1 lo fa solo abbaiare, il 27 lo mostra) e da nessun'altra parte.
 */
export function dog(P, x, y, s, color = 'rgba(9,11,15,0.92)') {
  const ctx = P.ctx;
  ctx.save();
  ctx.translate(x, y);
  ctx.fillStyle = color;
  ctx.fillRect(-s * 0.5, -s * 0.42, s, s * 0.3);              // corpo
  ctx.beginPath();                                            // testa
  ctx.arc(s * 0.56, -s * 0.46, s * 0.17, 0, Math.PI * 2);
  ctx.fill();
  ctx.beginPath();                                            // muso
  ctx.moveTo(s * 0.6, -s * 0.5); ctx.lineTo(s * 0.86, -s * 0.44); ctx.lineTo(s * 0.6, -s * 0.38);
  ctx.closePath(); ctx.fill();
  ctx.beginPath();                                            // orecchio
  ctx.moveTo(s * 0.48, -s * 0.6); ctx.lineTo(s * 0.58, -s * 0.74); ctx.lineTo(s * 0.62, -s * 0.56);
  ctx.closePath(); ctx.fill();
  for (const dx of [-0.38, -0.16, 0.2, 0.4]) ctx.fillRect(dx * s, -s * 0.14, s * 0.075, s * 0.15);
  ctx.fillRect(-s * 0.62, -s * 0.56, s * 0.16, s * 0.07);     // coda bassa
  ctx.restore();
}
